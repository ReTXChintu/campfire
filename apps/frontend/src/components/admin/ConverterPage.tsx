...
export default function ConverterPage() {
  ...
  const startConvert = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiPost<{ jobId: string }>("/api/admin/convert", {
        stagingId,
            ...
            {stage === "done" && downloadId && (
              <div className="flex flex-col gap-3">
                <button
                  ...
                  onClick={() =>
                    downloadAuthedFile(
                      `/api/admin/download/${downloadId}?filename=${encodeURIComponent(downloadFilename ?? "video.mp4")}`,
                    ...
                    {subtitleTracks.map((track) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            downloadAuthedFile(
                              `/api/admin/subtitle-sets/${subtitleSetId}/download/${track.index}`,
                              `${label}.vtt`,
                            )
                          }
